...
University::University(const University& other) {
	...
	for (int i = 0; i < other.number_of_student; ++i) {		//Бежим по всему второму списку
		//Создаем в текущем списке элемент, передавая в конструктор данные из второго списка
...
University::~University() {
	...
	for (int i = 0; i < number_of_student; ++i) {
		current = head_student;									//Запоминаем первый элемент
...
istream& operator>>(istream& in, University& obj) {
	...
	system("cls");
	Student* tmp = new Student;							//Создаем новый элемент списка
	++obj.number_of_student;							//Увеличиваем количество элементов в списке

	if (obj.head_student == nullptr) {					//Если элемент первый
		obj.head_student = tmp;							//То это начало списка
	}
	else {												//Иначе
...
void University::delete_student() {
	...
	try {
		...
		cout << "Введите фамилию: ";							//Ввод фамилии студента
		cin >> surname;
		cout << "Введите инициалы: ";							//Ввод инициалов студента
		cin >> initials;

		Student* current = head_student;						//Указатель на текущий элемент
		Student* prev = head_student;							//Указатель на предыдущий элемент
		for (int i = 0; i < number_of_student; ++i) {
			if (current->get_surname() == surname && current->get_initials() == initials) {		//Ищем подходящего студента
				if (current == head_student) {													//Если удаляемый элемент первый
					head_student = head_student->get_next_student();							//Передвигаем указатель на начало списка на второй элемент
					delete current;																//Удаляем первый элемент
					--number_of_student;														//Уменьшаем количество элементов
				}
				else {																			//Если не первый
					prev->set_next_student(current->get_next_student());						//Предыдущий указываетн на следующий элемент
					delete current;																//Удаляем текущий элемент
					--number_of_student;														//Уменьшаем количество элементов
				}
				cout << "Студент успешно удален!" << endl << endl;
				break;																			//Если нашли и удалили, то искать дальше нет смысла
			}

			if (i == number_of_student - 1)														//Если мы дошли до последнего элемента, но так и не нашли нужного студента
				cout << "Такого студента нет!" << endl << endl;

			prev = current;																		//Запоминаем элемент как предыдущий
			current = current->get_next_student();												//Передвигаем указатель
		}
		system("pause");
	}
	catch (exception& ex) {
...
void University::sort_student() { //алгоритм сортировка, сложность n^2
	...
	for (int i = 0; i < number_of_student - 1; ++i) {
		Student* current = head_student;									//Текущий элемент
		Student* prev = head_student;										//Предыдущий элемент
		while (current->get_next_student() != nullptr) {					//Идем, пока существует следующий элемент
			if (current->get_average_mark() > current->get_next_student()->get_average_mark()) {	//Сравнение среднего балла
				Student* next = current->get_next_student();				//Следующий элемент	
				if (current == head_student) {								//Если текущий элемент первый
					head_student = next;									//Начало списка теперь второй элемент
					current->set_next_student(next->get_next_student());	//Первый элемент тепер указывает на третий элемент
					next->set_next_student(current);						//Второй элемент теперь указывает на первый элемент
					current = head_student;					// Сохраняем позицию указателя чтобы после сравнения 1 и 2, мы сравнили 2 и 3 
				}
				else {
					prev->set_next_student(next);							//Предыдущий указывает на следующий элемент
					current->set_next_student(next->get_next_student());	//Текущий указывает на элемент после следующго
					next->set_next_student(current);						//Следующий указывает на текущий элемент
					current = next;											// Сохраняем позицию указателя (на элемент назад сдвигаем)
				}
			}

			prev = current;												//Иначе просто передвигаем указатели
			current = current->get_next_student();
