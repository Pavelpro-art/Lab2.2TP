...
Student::Student(string surname, string initials, string group, Disciplines* head_disciplines) {
	...
	Disciplines* cur = head_disciplines;							//Указатель, чтобы бегать по второму списку дисциплин

	while (cur != NULL) {											//Пока второй список не закончился, добавляем дисциплины в текущий список
...
Student::~Student() {
	...
	for (int i = 0; i < number_of_disciplines; ++i) {
		current = head_disciplines;									//Запоминаем первый элемент
...
//Аналогично тому, как было в конструкторе с параметром, только ввод данных с клавиатуры
...
void Student::delete_disciplines(string name) {
	...
	for (int i = 0; i < number_of_disciplines; ++i) {
		if (current->name == name) {										//Ищем дисциплину с нужным названием
			if (current == head_disciplines) {								//Если первый элемент
				head_disciplines = head_disciplines->next_disciplines;		//Переносим начало на второй элемент
				delete current;												//Удаляем певый элемент						
			}
			else {
			...
			average_score();												//Перерасчет среднего балла
			break;
		}
